...
*   **Binder 内存限制与池大小**：
    *   普通进程的 Binder 缓冲区约为 **1MB - 8KB**，由整个进程共享。
    *   **Binder 线程池**：默认最大 16 个线程。若服务端 16 个线程全部被耗时任务占满，后续 IPC 请求会阻塞，表现为 App 无响应。
*   **ServiceManager**：
...
*   **为什么旋转屏幕不销毁？**
    ...
    *   在 Activity 销毁前，`onRetainNonConfigurationInstance` 被调用，`ViewModelStore` 被存入 `NonConfigurationInstances` 中。新 Activity 启动后通过 `getLastCustomNonConfigurationInstance()` 找回。
*   **进程被动杀死后的恢复**：
...
*   **隔离机制**：
    *   `api` vs `implementation`：减少依赖链冲突，提升编译速度。
...
*   **ART 编译演进**：从 JIT (Android 5) -> 全量 AOT (Android 6) -> 混合编译 (Android 7+)。
